...
Usage:
  python -m templates.NjAllied.run <pdf> [<pdf> ...] [-o output.xlsx]

When multiple PDFs are given, their data is merged into one workbook with
one sheet per section. Every row carries book_year / book_month / book_label
so the app can split by edition.
"""
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="NjAllied PDF price-book scraper")
    parser.add_argument("pdfs", nargs="+", type=Path,
                        help="Path(s) to one or more Price Book PDFs")
    parser.add_argument("-o", "--out", type=Path, default=None,
                        help="Output .xlsx (default: 'Allied Price Book.xlsx' "
    ...
    out = args.out or (args.pdfs[0].parent / "Allied Price Book.xlsx")
